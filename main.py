...
async def run_bot_cycle():
    ...
    try:
        ...
        try:
            ...
            if oc_data:
                ...
                if oc_data.get('strikes'):
                    ...
                    send_result = await send_message(msg)
                    logger.info(f"Step 6: Message send result: {send_result}")
                    
                    if send_result:
                        logger.info("Step 7: ✅ Option chain message sent!")
                        oc_success = True
                    else:
                        logger.error("Step 7: ❌ Message send failed!")
        ...
        logger.info(f"Option chain completed. Success: {oc_success}")
        logger.info("="*60)
